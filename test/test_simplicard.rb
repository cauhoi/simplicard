...
class TestSimplicard < Minitest::Test
  def test_that_it_has_a_version_number
    refute_nil ::Simplicard::VERSION
  end
  ...
  def test_add_a_card
    ...
    card2 = Simplicard::Card.new(13,0)
  ...
  def test_shuffle
    ...
    random_card = @deck.my_deck[random]
    @deck.shuffle_it
    refute_equal first_card.value, @deck.my_deck.first.value
    refute_equal random_card.value, @deck.my_deck[random].value
  end
  ...
  def test_card_should_be_valid_when_created
    exception = assert_raises ArgumentError do
      bad_card = Simplicard::Card.new(14,2)
    end
  assert_equal("Card value is not valid", exception.message)
  exception = assert_raises ArgumentError do
      bad_card = Simplicard::Card.new(19,5)
    end
  ...
  def setup
    ...
    @card = Simplicard::Card.new(1,3)
    @empty_deck = Simplicard::CardDeck.new
